...
namespace WeChatClient.HTTP
{
    ...
    class LoginService
    {
        ...
        /// 获取二维码的URL  后面加会话id
        /// </summary>
        private static string _qrcode_url = "https://login.weixin.qq.com/qrcode/";

        /// <summary>
        /// 判断二维码扫描情况  200表示扫描登录  201表示已扫描未登录   其他表示未扫描   后面加会话id
        /// </summary>
        ...
        public object LoginCheck()
        {
            ...
            if (login_result.Contains("=201")) //已扫描未登录
            {//返回后面格式字符串 window.code=201;window.userAvatar = 'data:img/jpg;base64,/9j/4AAQSkZJRgABAQAASABIAAD/2wBDAAMCAgMCAgMDAwMEAwMEBQgFBQQEBQoHBwYIDAoMDAsKCwsNDhIQDQ4RDgsLEBYQERMUFRUVDA8XGBYUGBI
            ...
            else if (login_result.Contains("=200"))  //扫描已登录
            {//window.code=200;window.redirect_uri = "https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxnewloginpage?ticket=AWcf1dxcxzz36jMzkclxHJeK@qrticket_0&uuid=oasJI4vmNg==&lang=zh_CN&scan=1479369498";
